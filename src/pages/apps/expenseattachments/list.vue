...
const { data: companyData } = await useApi(
  createUrl(`${baseUrl}/expense_attachments`),
)
...
const deleteService = async id => {
  ...
  const res = await $api(`${baseUrl}/expense_attachments/${id}`, {
    method: "DELETE",
  })
  ...
  const { data: companyData } = await useApi(createUrl(`${baseUrl}/expense_attachments`))

  company.value = companyData.value
  router.push({ name: 'apps-expenseattachments-list' })
}
</script>
      ...
      <VCardText class="d-flex flex-wrap py-4 gap-4">
        ...
        <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
          ...
          <VBtn
            ...
            @click="$router.push({name:'apps-expenseattachments'})"
          >
      ...
      >
        ...
        <template #item.actions="{ item }">
          ...
          <IconBtn
            @click="
              $router.push({
                name: 'apps-expenseattachments-edit-id',
                params: { id: item.id }})
            "
          >

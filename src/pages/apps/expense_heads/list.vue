...
const { data: companyData } = await useApi(
  createUrl(`${baseUrl}/expense_heads`),
)
...
const deleteService = async id => {
  ...
  const res = await $api(`${baseUrl}/expense_heads/${id}`, {
    method: "DELETE",
  })
  ...
  const { data: companyData } = await useApi(createUrl(`${baseUrl}/expense_heads`))
      ...
      <VCardText class="d-flex flex-wrap py-4 gap-4">
        ...
        <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
          ...
          <VBtn
            ...
            @click="$router.push({name:'apps-expenseheads'})"
          >
      ...
      >
        ...
        <template #item.actions="{ item }">
          ...
          <IconBtn
            @click="
              $router.push({
                name: 'apps-expenseheads-edit-id',
                params: { id: item.id }})
            "
          >

...
const deleteService = async id => {
  ...
  const res = await $api(`${baseUrl}/banks/${id}`, {
    method: "DELETE",
  })
  ...
  const { data: companyData } = await useApi(createUrl(`${baseUrl}/banks`))
      ...
      <VCardText class="d-flex flex-wrap py-4 gap-4">
        ...
        <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
          ...
          <VBtn
            ...
            @click="$router.push({name:'apps-banks'})"
          >
            Add New Sub Banks
          </VBtn>
        </div>
